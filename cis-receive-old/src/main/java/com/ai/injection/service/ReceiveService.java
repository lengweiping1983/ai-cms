...
public class ReceiveService extends AbstractService<ReceiveTask, Long> {
	...
	public void handleTask(ReceiveTask receiveTask, InjectionPlatform platform,
		...
		Long currentCpId = (cp != null) ? cp.getId() : null;
	...
	private void saveSeries(SeriesBean seriesBean, ReceiveTask receiveTask,
			InjectionPlatform platform, Long currentCpId) {
		...
		if (StringUtils.isNotEmpty(seriesBean.getDirector())) {
			series.setDirectors(seriesBean.getDirector());
		}
		if (StringUtils.isNotEmpty(seriesBean.getActorDisplay())) {
			series.setActors(seriesBean.getActorDisplay());
		}
		if (StringUtils.isNotEmpty(seriesBean.getCompere())) {
			series.setComperes(seriesBean.getCompere());
		}
		if (StringUtils.isNotEmpty(seriesBean.getGuest())) {
			series.setGuests(seriesBean.getGuest());
		}
		...
		if (StringUtils.isNotEmpty(seriesBean.getTags())) {
			series.setTags(seriesBean.getTags());
		}
...
//		}
		series.setInjectionStatus(InjectionStatusEnum.INJECTION_SUCCESS.getKey());
		series.setInjectionTime(new Date());
		series.setPartnerItemCode(seriesBean.getCode());
		series.setPartnerPlayCodeStatus(PlayCodeStatusEnum.INPUT.getKey());
		try {
	...
	private void saveProgram(ProgramBean programBean, ReceiveTask receiveTask,
		...
		if (StringUtils.isNotEmpty(programBean.getDirector())) {
			program.setDirectors(programBean.getDirector());
		}
		if (StringUtils.isNotEmpty(programBean.getActorDisplay())) {
			program.setActors(programBean.getActorDisplay());
		}
		if (StringUtils.isNotEmpty(programBean.getCompere())) {
			program.setComperes(programBean.getCompere());
		}
		if (StringUtils.isNotEmpty(programBean.getGuest())) {
			program.setGuests(programBean.getGuest());
		}
		...
		if (StringUtils.isNotEmpty(programBean.getTags())) {
			program.setTags(programBean.getTags());
		}
...
//		}
		program.setInjectionStatus(InjectionStatusEnum.INJECTION_SUCCESS.getKey());
		program.setInjectionTime(new Date());
		program.setPartnerItemCode(programBean.getCode());
		program.setPartnerPlayCodeStatus(PlayCodeStatusEnum.INPUT.getKey());
		try {
...
//		}
		mediaFile.setInjectionStatus(InjectionStatusEnum.INJECTION_SUCCESS.getKey());
		mediaFile.setInjectionTime(new Date());
		mediaFile.setPartnerItemCode(movieBean.getCode());
//		program.setPartnerPlayCodeStatus(PlayCodeStatusEnum.INPUT.getKey());
