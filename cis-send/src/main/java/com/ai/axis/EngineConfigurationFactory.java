...
public class EngineConfigurationFactory extends
	...
	public static org.apache.axis.EngineConfigurationFactory newFactory(
			Object param) {
		// see comments inside method newFactory in
		// org.apache.axis.configuration.EngineConfigurationFactoryServlet
		return (param instanceof ServletConfig) ? new EngineConfigurationFactory(
				(ServletConfig) param) : null;
	}
	 ...
	 * org.apache.axis.configuration.EngineConfigurationFactoryServlet except it
	 * also tries to find wsdd config file in classpath.
	 *
	...
	public EngineConfiguration getServerEngineConfig() {
		...
		if (config == null) {
			...
			try {
				InputStream is = ClassUtils.getResourceAsStream(
						AxisServer.class, SERVER_CONFIG_FILE);
				config = new FileProvider(is);
			} catch (Exception e) {
