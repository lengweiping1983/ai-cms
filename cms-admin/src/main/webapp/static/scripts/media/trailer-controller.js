$(function() {
	$.TrailerController = function(elm, config) {

	}

	$.extend($.TrailerController, {
		...
		toDelete : function(path, name) {
			...
			json.event = "$.TrailerController._delete('" + path + "')";
		...
		detail : function(id) {
			$.common.showModal({
				url : contextPath + "/media/trailer/" + id + "/detail",
				success : function() {
		...
		toChangeStatus : function(path, name, statusMethodDesc) {
			...
			json.event = "$.TrailerController.changeStatus('" + path + "')";
			$.common.showConfirmModal(json);
		},
		...
		showProgramList : function(itemId, itemType, itemName, managementName) {
			$.common.putLastContentParam();

			$.common.ajaxLoadContent({
				url : contextPath + "/media/trailer",
				type : "GET",
				dataType : "text",
				contentType : "default",
				data : {
					search_itemType__EQ_I : itemType,
					search_itemId__EQ_L : itemId,
					itemName : itemName,
