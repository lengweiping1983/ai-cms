...
var PlaylistMenuItem = function (_Component) {
  ...
  PlaylistMenuItem.prototype.createEl = function createEl() {
    ...
    var upNextText = this.localize('下一条播放');
    ...
    // 码率
    var bitrateEl = document_1.createElement('span');
    var bitrate = this.localize('未知');
    if (item.bitrate) {
    	bitrate = item.bitrate + 'K';
    }
    var bitrateText = '码率' + bitrate;

    bitrateEl.className = 'vjs-bitrate-text';
    bitrateEl.appendChild(document_1.createTextNode(bitrateText));
    bitrateEl.setAttribute('title', bitrateText);
    titleContainerEl.appendChild(bitrateEl);

    // Video title
    var titleEl = document_1.createElement('cite');
    var titleText = item.name || this.localize('未知');
