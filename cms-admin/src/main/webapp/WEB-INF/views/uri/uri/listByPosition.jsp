		...
		<div class="col-md-12">
			<div class="portlet box green">
				<div class="portlet-title">
					<div class="caption">
						<i class="fa fa-globe"></i>[${uri.title}]页面数据编排
					</div>
					<div class="actions">
				...
				<div class="portlet-body">
					...
					<c:if test="${! empty template.backgroundImage}">
						<c:set var="backgroundImage" value="${template.backgroundImage}" />
					</c:if>
					<c:if test="${! empty uri.backgroundImage}">
						<c:set var="backgroundImage" value="${uri.backgroundImage}" />
					</c:if>
					<input type="hidden" id="editType" name="editType" value="uri" />
					<input type="hidden" id="uriId" name="uriId"
						value="${uri.id}" /> <input type="hidden" id="positionStr"
						name="positionStr" value='${positionStr}' /> <input type="hidden"
						id="backgroundImage" name="backgroundImage"
						value="${backgroundImage}">
					<input type="hidden" id="internalParam" name="internalParam" value="${param.internalParam}">
					...
					<c:if test="${! empty backgroundImage}">
						<c:set var="imageUrl"
							value="${fns:getImagePath(backgroundImage)}" />
					</c:if>
