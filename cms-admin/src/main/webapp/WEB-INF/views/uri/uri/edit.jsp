				...
				<h4 class="modal-title">
					<c:choose>
						...
						<c:otherwise>
							...
							<c:set var="appCode" value="${uri.appCode}" />
						</c:otherwise>
					</c:choose>
					${methodDesc}页面
				</h4>
						...
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									...
									<div class="col-md-6">
										<input type="hidden" id="image1" name="image1" value="${uri.image1}">

										<c:if test="${empty uri.image1}">
											<c:set var="imageUrl" value="${ctx}/static/img/no-image.png" />
											<c:set var="fileinputMethod" value="fileinput-new" />
										</c:if>
										<c:if test="${! empty uri.image1}">
											<c:set var="imageUrl" value="${fns:getImagePath(uri.image1)}" />
								...
								<div class="form-group">
									...
									<div class="col-md-6">
										<input type="hidden" id="image2" name="image2"
											value="${uri.image2}">
										<c:if test="${empty uri.image2}">
											<c:set var="imageUrl" value="${ctx}/static/img/no-image.png" />
											<c:set var="fileinputMethod" value="fileinput-new" />
										</c:if>
										<c:if test="${! empty uri.image2}">
											<c:set var="imageUrl"
												value="${fns:getImagePath(uri.image2)}" />
											<c:set var="fileinputMethod" value="fileinput-exists" />
										</c:if>
				...
				<c:choose>
					<c:when test="${empty uri.id}">
						<button class="btn btn-outline green"
							onclick="$.UriController.edit('${ctx}/uri/uri/add');">
							<i class="fa fa-save"></i>保存
						</button>
					</c:when>
					<c:otherwise>
						<button class="btn btn-outline green"
							onclick="$.UriController.edit('${ctx}/uri/uri/${uri.id}/edit');">
							<i class="fa fa-save"></i>保存
						</button>
					</c:otherwise>
				</c:choose>
