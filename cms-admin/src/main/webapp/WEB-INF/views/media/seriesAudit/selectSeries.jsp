...
<c:set var="formId" value="selectSeries" />
<div class="table-container">
	<div class="table-actions-wrapper-condition">
		<form action="${ctx}/media/seriesAudit/selectSeries" id="selectSeries">
		...
		<c:forEach items="${page.content}" var="t" varStatus="status">
			<tr>
				...
				<td><c:choose>
						...
						<c:otherwise>
							<a href="javascript:;"
								onclick="$.SeriesController.selectSeries('${param.selectMode}','${param.selectParam}','${t.id}','${t.name}','${t.title}','${t.contentType}','${t.tag}','${t.internalTag}','${t.episodeTotal}','${t.duration}','${t.status}','${t.cpCode}','${t.filename}','${t.image1}','${t.image2}','${t.image3}','${t.image4}');">${t.name}</a>
						</c:otherwise>
					</c:choose></td>
				<td>${t.title}</td>
	...
	if ('${param.selectMode}' == 'multi') {
		if ($("#content_list_modal_container_ok")[0]) {
			$('#content_list_modal_container_ok').attr("onclick",
					"$.SeriesController.selectCheckedItem('${formId}', 2);");
