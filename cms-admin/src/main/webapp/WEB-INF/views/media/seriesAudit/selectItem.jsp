		...
		<c:forEach items="${page.content}" var="t" varStatus="status">
			<tr>
				...
				<td><c:choose>
						...
						<c:otherwise>
							<a href="javascript:;"
								onclick="$.SeriesController.selectItem('${param.selectMode}','${param.selectParam}','${t.id}','${t.name}','${t.title}','${t.contentType}','${t.tag}','${t.internalTag}','${t.episodeTotal}','${t.duration}','${t.status}','${t.cpId}','${t.filename}','${t.image1}','${t.image2}','${t.image3}','${t.image4}');">${t.name}</a>
						</c:otherwise>
					</c:choose></td>
				<td>${t.title}</td>
