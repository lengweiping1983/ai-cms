				...
				<form id="editForm">
					<div class="content form-horizontal">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label col-md-3">总集数: </label>

									<div class="col-md-9">
										<input type="text" id="episodeTotal" name="episodeTotal"
											value="${transcodeRequest.episodeTotal}" class="form-control"
											placeholder="请输入总集数" />
									</div>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label col-md-3">生成节目名称规则(<span
										class="required">*</span>):
									</label>

									<div class="col-md-9">
										<select id="genProgramNameRule" name="genProgramNameRule"
											class="form-control validate[required]">
											<option value="">请选择</option>
											<c:forEach var="item" items="${genProgramNameRuleEnum}">
												<option value="${item.key}"
													<c:if test="${item.key eq transcodeRequest.genProgramNameRule}"> selected="selected" </c:if>>${item.value}</option>
											</c:forEach>
										</select>
									</div>
								</div>
									...
									<tbody id="transcodeRequest_file_list">
										<c:forEach var="file" items="${transcodeRequest.fileList}">
											<tr id="tr_file_${file.id}" class="tr_file_css">
												...
												<td><input type="text" name="episodeIndex"
													value="${file.episodeIndex}"
													class="form-control input-small input-inline validate[required,custom[integer]]">
												</td>
