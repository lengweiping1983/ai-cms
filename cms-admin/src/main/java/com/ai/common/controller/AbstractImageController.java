...
public abstract class AbstractImageController extends AbstractController {
    ...
    public void deleteOldResource(String sourceImageUrl) {
        ...
        File file = new File(StringUtils.trimToEmpty(AdminGlobal.imageUploadPath) + sourceImageUrl);
        if (file.exists()) {
            file.delete();
        }
    }
    ...
    public static String getImageOutputDirPath(String module, String type) {
        ...
        if (StringUtils.isNotEmpty(AdminGlobal.getSiteCodePath())) {
            dirPath = "/" + StringUtils.trimToEmpty(AdminGlobal.getSiteCodePath()) + dirPath;
        }
        return dirPath;
    }

    public String upload(String module, String type, String imageData) {
        if (StringUtils.isEmpty(module) || StringUtils.isEmpty(type) || StringUtils.isEmpty(imageData)) {
            return "";
        }
        ...
        try {
            ...
            if (null != data && data.length > 1) {
                ...
                String imageUploadPath = StringUtils.trimToEmpty(AdminGlobal.imageUploadPath);
